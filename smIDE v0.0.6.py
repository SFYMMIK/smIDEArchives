...
class smIDE_Version:
  def __init__(self):
    self.label = tk.Label(root, text="Checking smIDE's version...")
    self.label.pack(pady=10)

    self.ver_url = "https://raw.githubusercontent.com/ELO222565/ver/main/version_"

    self.smIDE_local_ver= '0.0.5'

    self.check_smIDE_version()

  def get_remote_ver(self):
    try:
      response = requests.get(self.ver_url)
      response.raise_for_status()
      smIDE_Version_match = re.search(r'__version__ = \'(.+?\'', response.text)
      if smIDE_Version_match:
        return smIDE_Version_match.group(1)
    except requests.exceptions.RequestException as e:
      messagebox.showerror("Error," f"Error Fetching Version: {e}")
      return None

  def check_smIDE_version(self):
    remote_ver = self.get_remote_ver()

    if remote_ver is not None:
      if self.smIDE_local_ver == remote_ver:
        self.label.config(text="Everything is in place. You are using the latest version of smIDE.")
      else:
        self.label.config(text=f"smIDE is out of date. Your version is {self.smIDE_local_ver}. Please Donwload the latest version")
        if choice == 'yes':
          self.open_smIDE_GitHub()

    else:
      self.label.config(text="Error Fetching Version")

def open_smIDE_GitHub(self):
  smIDE_Version_url = 'https://raw.githubusercontent.com/ELO222565/ver/main/version_'
  webbrowser.open_new(smIDE_Version_url)

class smIDE:

  def __init__(self, root):
    self.root = root
    self.root.title("smIDE DevBeta ver0.0.5")
    ...
    if self.current_tab:
      if hasattr(self, 'current_extension') and self.current_extension:
        file_path = filedialog.asksaveasfilename(
            defaultextension=f".{self.current_extension}",
            filetypes=[(f"{self.current_extension.upper()} files",
                        f"*.{self.current_extension}"), ("All files", "*.*")])
        if file_path:
