...
class smIDE:
    def __init__(self, root):
        self.root = root
        self.root.title("smIDE DevBeta ver0.0.3")
        self.root.geometry("1250x1000")
        self.root.option_add('*Font', 'helvetica 11')  # Set a custom font
    ...
    def create_notebook(self):
        ...
        for lang in languages:
            text_widget = scrolledtext.ScrolledText(self.notebook, wrap=tk.WORD, bg='#282c34', fg='white', insertbackground='white', insertwidth=4, padx=10, pady=10)
        ...
        self.notebook.bind("<ButtonRelease-1>", self.on_tab_selected)  # Bind event to tab selection
    ...
    def create_file_explorer(self):
        ...
        self.file_listbox = tk.Listbox(frame, selectmode=tk.SINGLE, bg='#282c34', fg='white', selectbackground='#61dafb', font=('helvetica', 11))
    ...
    def create_toolbar(self):
        ...
        plus_button = ttk.Button(toolbar, text="+", command=self.create_new_tab)
    ...
    def on_tab_selected(self, event):
        ...
        extension_mapping = {"HTML": "html", "CSS": "css", "SCSS": "scss", "JavaScript": "js", "Python": "py"}
        ...
        if selected_index:
            ...
            if os.path.isfile(file_path):
                ...
                with open(file_path, "r") as file:
                    ...
                    formatter = get_formatter_by_name("html", linenos=True, cssclass="source")
    ...
    def save_file(self):
        if self.current_tab:
            if hasattr(self, 'current_extension') and self.current_extension:
                file_path = filedialog.asksaveasfilename(defaultextension=f".{self.current_extension}", filetypes=[(f"{self.current_extension.upper()} files", f"*.{self.current_extension}"), ("All files", "*.*")])
                if file_path:
                ...
                messagebox.showwarning("Save Error", "Please select a language tab first.")
        else:
    ...
    def create_new_tab(self):
        ...
        new_tab_name = simpledialog.askstring("New Tab", "Enter the name for the new tab:", parent=self.root)
        if new_tab_name:
            # Prompt user to select a language for the new tab
            language_options = ["Python", "HTML", "CSS", "SCSS", "JavaScript", "Undefined"]
            selected_language = simpledialog.askstring("New Tab", "Select a language for the new tab:", parent=self.root, initialvalue="Python",
                                                      show=list(map(lambda x: x.lower(), language_options)))
            if selected_language:
                ...
                text_widget = scrolledtext.ScrolledText(self.notebook, wrap=tk.WORD, bg='#282c34', fg='white', insertbackground='white', insertwidth=4, padx=10, pady=10)
                text_widget.config(font=('helvetica', 12))
                self.notebook.add(text_widget, text=f"{new_tab_name} ({selected_language})")
                self.text_widgets[new_tab_name] = text_widget
                self.notebook.select(len(self.text_widgets) - 1)  # Select the last tab
                self.update_tab_count(f"{new_tab_name} ({selected_language})")  # Update tab count
